...
function Node() {
	this.info = {
		...
		protocol: null,
		api: null,
		port: (process.env.LISTENING_PORT || 18130),
		os: os.platform(),
		os_v: os.release(),
...
Node.prototype.startWeb3Connection = function() {
	...
	web3 = new Web3();
	web3.setProvider(new web3.providers.HttpProvider('http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '1234')));
	this.checkWeb3Connection();
}

Node.prototype.rpcRequest = async function (method, params) {
	let options = {
		url: process.env.QNG_RPC_HOST,
		method: "post",
		headers:
			{
		...
		auth: {
			user: process.env.QNG_RPC_USER,
			pass: process.env.QNG_RPC_PASS
		},
		rejectUnauthorized: false,
		body: JSON.stringify({ "jsonrpc": "2.0", "id": "nodejs", "method": method, "params": params })
	};
...
Node.prototype.setupSockets = function() {
	...
	socket.on('open', async function open() {
		...
		let peersBody = await self.rpcRequest("getPeerInfo", [true]);
		let peersR = JSON.parse(peersBody);
		peersR = peersR.result;
		// monitor more peers to send hello
		for (let i = 0; i < peersR.length; i++) {
			let info = {};
			let result = peersR[i];
			console.log(result)
			info.name = result.name + "/" + result.services + "/" + result.network + "/" + result.version + "/" + result.graphstate.mainorder;
			info.coinbase = result['id'];
			info.node = result.address;
			info.net = result.network;
			info.protocol = result.protocol;
			info.api = result.address;
			let b = result.address.split("/");
			info.ip = b[1];
			info.gsupdate = result.gsupdate;
...
Node.prototype.getInfo = async function () {
	...
	try {
		let body = await this.rpcRequest("getNodeInfo", []);
		console.timeEnd('Got info');
		let result = JSON.parse(body);
		result = result.result;
		this.info.name = "qng/" + result.network + "/" + result.buildversion;
		this.info.coinbase = result['ID'];
		this.info.node = result.address[result.address.length - 1];
		this.info.net = result.network;
		this.info.protocol = result.protocolversion + "/" + result.buildversion + "/" + result.version;
		this.info.api = result.address[result.address.length - 1];
